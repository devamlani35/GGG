...
class ImDataset():
    ...
    def __init__(self,num_files = 167585, mode="part1", db_table="data_paths", db_host="gnome-1.cwqqmyn32oy5.us-east-1.rds.amazonaws.com", bucket_arn="arn:aws:s3-object-lambda:us-east-1:068469771374:accesspoint/s3-lambda-3c2", n_epochs=50):
        self.batch_size = 10
        ...
        self.arn = bucket_arn
        self.n_batches = num_files//self.batch_size
        self.batch_holder = []
        self.thread_arr = []
        self.n_epoches=n_epochs
        thread = threading.Thread(target=self.start_querying)
        thread.start()
        
    def start_querying(self):
        for i in range(self.n_batches):
            self.thread_arr = [th for th in self.thread_arr if th.is_alive()]
            while len(self.thread_arr)>=10:
                time.sleep(0.1)
            else:
                temp = threading.Thread(target=self.handle_threaded_query, args=(i,))
                temp.start()
                self.thread_arr.append(temp)

    def handle_threaded_query(self,idx):
        start=idx*self.batch_size
        self.thread_arr.append(self.query_db(self.train_indices[start:start+self.batch_size]))
    ...
    def __getitem__(self, idx):
        while(len(self.batch_holder) <1):
            time.sleep(0.1)
        batch = self.batch_holder.pop(0)
        return batch
